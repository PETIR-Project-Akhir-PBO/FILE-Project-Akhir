...
public class PenumpangDAO {
    ...
    public Penumpang getById(int id) {
        String sql = "SELECT p.id_pengguna, p.nama, p.password, p.no_telp, pn.saldo FROM pengguna p LEFT JOIN penumpang pn ON p.id_pengguna=pn.id_pengguna WHERE p.id_pengguna=?";
        try (Connection c = DatabaseConnection.getConnection();
            ...
            if (rs.next()) {
                ...
                return new Penumpang(rs.getInt("id_pengguna"), rs.getString("nama"), rs.getString("password"), rs.getString("no_telp"), saldo);
            }
