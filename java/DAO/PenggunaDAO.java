...
public class PenggunaDAO {
    ...
    // Konstruktor baru: terima koneksi dari Service
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            if (rs.next()) {
                ...
                try (PreparedStatement p2 = conn.prepareStatement(
                        "SELECT saldo FROM penumpang WHERE id_pengguna=?")) {
                    p2.setInt(1, id);
                    ResultSet r2 = p2.executeQuery();
                    if (r2.next()) saldo = r2.getInt("saldo");
                    else {
                        try (PreparedStatement ins = conn.prepareStatement(
                                "INSERT INTO penumpang (id_pengguna, saldo) VALUES (?, 0)")) {
                            ins.setInt(1, id);
                            ins.executeUpdate();
                            saldo = 0;
                        }
                ...
                return new Penumpang(id, rs.getString("nama"),
                        rs.getString("password"), noTelp, saldo);
            }
    ...
    public int daftar(String nama, String password, String noTelp) {
        ...
        try (PreparedStatement ps = conn.prepareStatement(cek)) {
            ...
            ResultSet rs = ps.executeQuery();
            if (rs.next()) return -2; // nomor sudah digunakan
        } catch (SQLException e) {
            e.printStackTrace();
            return -1;
        }
